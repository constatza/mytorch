...
from mytorch.networks.utils import atleast_2d, recursive_apply
from mytorch.networks.utils import conv_kernel, conv_kernel_transpose
from mytorch.networks.utils import conv_out, conv_out_repeated, conv_out_repeated_2d
from mytorch.networks.utils import conv_out_transpose
from mytorch.networks.utils import conv_out_vect
...
def test_conv_out_transpose(args):
    input_size, kernel_size, stride, padding, expected_output_size = args
    assert (
        conv_out_transpose(input_size, kernel_size, stride, padding)
...
def test_conv_kernel_transpose(args):
    ...
    input_size, output_size, stride, padding, expected_output = args
    assert (
        conv_kernel_transpose(input_size, output_size, stride, padding)
        == expected_output
    )
...
def test_conv_output_repeated(args, expected_output):
    ...
    assert (
        conv_out_repeated(input_size, kernel_size, stride, padding, num_reps=num_reps)
        == expected_output
    )
...
def test_conv_output_repeated_2d():
    ...
    assert conv_out_repeated_2d(
        input_size, kernel_size, stride, padding, num_reps=num_reps
    ) == (100, 100)
