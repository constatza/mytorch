...
from mytorch.io.config import ScenarioConfig, PathsOutputConfig
            ...
            for train_id, training_parameters in self.training_combinations.items():
                ...
                if experiment_id not in self.experiments_ran:
                    ...
                    model = model_parameters["model"](
                        **self.model_shape_parameters, **model_parameters
                    )
                    ...
                    try:
                        ...
                        logger.log(
                            f"Experiment {experiment_id:d} completed successfully."
                        )
                        ...
                        logger.write(
                            f"{experiment.name}.toml", parameters, dirname="parameters"
                        )
                        logger.write(f"losses", val_loss[-1])
                    except Exception as e:
                        raise e
                        logger.error(
                            f"Experiment {experiment_id} failed with error: {e}"
                        )


if __name__ == "__main__":
    from mytorch.io.readers import read_scenario

    config = read_scenario(
        r"C:\Users\cluster\constantinos\mytorch\scenarios\bio-surrogate\config\u-cae.toml"
    )
